//
// > Notice: Floating Trees Inc. @ 2025
// > Create Time: 2025-07-05 19:33:34
//

#pragma once

#include "KGPU_CommandQueue.h"

#include <Metal/Metal.h>

namespace KGPU
{
    class Metal3Device;

    class Metal3CommandQueue : public ICommandQueue
    {
    public:
        Metal3CommandQueue(Metal3Device* device, CommandQueueType type);
        ~Metal3CommandQueue();

        ICommandList* CreateCommandList(bool singleTime) override;
        void CommitCommandList(ICommandList* cmdBuffer) override;
        void Wait() override;
        
        id<MTLCommandQueue> GetMTLCommandQueue() { return mCommandQueue; }
    private:
        Metal3Device* mParentDevice;

        id<MTLCommandQueue> mCommandQueue = nullptr;
    };
}
