...
namespace KGPU
{
    Metal3TLAS::Metal3TLAS(Metal3Device* device)
        : mParentDevice(device)
    {
        ...
        mDescriptor.instanceDescriptorBuffer = static_cast<Metal3Buffer*>(mInstanceBuffer)->GetMTLBuffer();
        ...
        device->GetResidencySet()->WriteBuffer(reinterpret_cast<Metal3Buffer*>(mInstanceBuffer));
        device->GetResidencySet()->WriteBuffer(reinterpret_cast<Metal3Buffer*>(mBindlessBuffer));

        KD_WHATEVER("Created Metal3 TLAS");
    }
    
    Metal3TLAS::~Metal3TLAS()
    {
        ...
        mParentDevice->GetResidencySet()->FreeBuffer(reinterpret_cast<Metal3Buffer*>(mInstanceBuffer));
        mParentDevice->GetResidencySet()->FreeBuffer(reinterpret_cast<Metal3Buffer*>(mBindlessBuffer));
        if (mBindlessBuffer) KC_DELETE(mBindlessBuffer);
        if (mInstanceBuffer) KC_DELETE(mInstanceBuffer);
        if (mScratch) KC_DELETE(mScratch);
    }

    void Metal3TLAS::ResetInstanceBuffer()
    {
        mBLASMap.clear();
        mInstances.clear();
        mInstanceCount = 0;
    ...
    void Metal3TLAS::AddInstance(IBLAS* blas, const KGPU::float4x4& transform, bool opaque)
    {
    ...
    uint64 Metal3TLAS::Address() const
    {
        return 0;
    }
}
