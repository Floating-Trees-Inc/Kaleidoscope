...
namespace KGPUT
{
    class DrawLinesIndirect : public BaseTest
    {
    public:
        DrawLinesIndirect()
        {
            ...
            Gfx::ShaderManager::SubscribeGraphics("data/kd/shaders/tests/draw_lines.kds", desc);
        ...
        void Execute() override
        {
            ...
            uint drawCommands[] = { 0, 4, 1, 0, 0 };
            Gfx::Uploader::EnqueueBufferUpload(drawCommands, sizeof(uint) * 5, mIndirectBuffer, mCommandList);

            auto* pipeline = Gfx::ShaderManager::GetGraphics("data/kd/shaders/tests/draw_lines.kds");
...
DEFINE_RHI_TEST(DrawLinesIndirect) {
    KGPUT::DrawLinesIndirect test;
    return test.Run();
}
